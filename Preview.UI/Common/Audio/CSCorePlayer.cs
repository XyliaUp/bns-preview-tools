...
public class CSCorePlayer : IDisposable
{
	...
	public void Play(byte[] data, string ext, AudioDevice audioDevice)
	{
		this.isStoppedBecausePaused = false;
		this.SetSelectedAudioDevice(audioDevice);
		...
		this.InitializeSoundOut(this.GetCodec(data, ext));
		this.ApplyFilter(this.filterValues);
		this.soundOut.Play();
	}

	private IWaveSource GetCodec(byte[] data, string ext)
	{
		this.audioStream = new MemoryStream(data);
		IWaveSource waveSource = new CustomCodecFactory().GetCodec(this.audioStream, ext);
	...
	public Equalizer Create10BandEqualizer(ISampleSource source)
	{
		return this.Create10BandEqualizer(source, 18, 0);
	}

	public Equalizer Create10BandEqualizer(ISampleSource source, int bandWidth, int defaultGain)
	{
