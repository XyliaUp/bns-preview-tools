...
{
	#region Icon 
	public static Bitmap Icon(this Skill3 skill) => skill.IconTexture.GetIcon(skill.IconIndex);
	...
	public static ItemIconCell GetObjIcon(this BaseRecord Object, short StackCount = 0)
	{
		...
		else if (Object is Skill3 skill) Image = skill.Icon();
		else return null;
	...
	{
		Dictionary<Type, Type> temp = new();

		temp.Add(typeof(Item) , typeof(ItemTooltipPanel));
		temp.Add(typeof(Skill3), typeof(SkillFrm));
	}
	...
	public static void PreviewShow(this BaseRecord obj, IWin32Window window = null)
	{
		var thread = new Thread(() =>
		{
			...
			else if (obj is Item item) ResultFrm = new ItemTooltipPanel(item);
			else if (obj is Skill3 skill) ResultFrm = new SkillFrm(skill);
			#endregion


			if (ResultFrm is null) Trace.WriteLine($"invalid window ({obj})");
			else if (window is null) ResultFrm.ShowDialog();
