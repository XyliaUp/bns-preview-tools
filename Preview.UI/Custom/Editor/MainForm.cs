...
public partial class MainForm : Form
{
	...
	private void CustomRTB_DragDrop(object sender, DragEventArgs e)
	{
		...
		if (fname != null)
		{
			if (fname is string[] list && !string.IsNullOrWhiteSpace(list[0]))
			{
	...
	private void SearchTextBox_TextChanged(object sender, EventArgs e)
	{
		...
		Wait.Tick += delegate
		{
			if ((DateTime.Now - Orginial).Seconds >= 2)
			{
				...
				var list = richOut.Highlight(SearchTextBox.Text, ps.HighlightsColor, chkMatchCase.Checked, chkMatchWholeWord.Checked);
			}
		};
		Wait.Start();
	}
	...
	{
		SaveContent?.Invoke(sender, new EventArgs());//把按钮自身作为参数传递//把按钮自身作为参数传递
	}
	#endregion
}