...
internal sealed class RewardPage
{
	...
	public Data.Record.Reward DecomposeReward;
	...
	private void CreateFixed()
	{
		...
		{
			var min = this.Attrs[$"fixed-item-min", num].ToShort();
			var max = this.Attrs[$"fixed-item-max", num].ToShort();
	...
	private void CreateSelected()
	{
		Create(64, (num) => Attrs[$"selected-item", num], (num) =>
		{
			...
			c.Params[3] = this.Attrs[$"selected-item-count", num].ToShort();
