	...
	public partial class Game_MapScene : PreviewFrm
	{
		...
		public void LoadData(string Rule)
		{
			...
			if (_mapInfo.ParentMapinfo != null)
			{
				...
				this.OpenParentMap.Click += new EventHandler((o, e) => Execute.MyShowDialog(new Game_MapScene(_mapInfo.ParentMapinfo.alias)));
			}
		...
		{
			var MapUnits = FileCache.Data.MapUnit.Where(o => o.Mapid == CurMapInfo.Key() && o.MapDepth <= _mapDepth);
			foreach (var mapunit in MapUnits)
			{
				...
				if (mapunit.Type == TypeSeq.Attraction)
				{
					...
					if (obj is Attraction attraction) tooltip += "\n" + attraction.GetDescribe();
				}
				else if (mapunit.Type == TypeSeq.Npc || mapunit.Type == TypeSeq.Boss)
				{
