...
namespace Xylia.Preview.GameUI.Scene.Game_ItemGrowth2
{
	...
	public partial class ResultWeaponPreview : UserControl
	{
		...
		{
			var PreviewResult = Recipes.Select(r => r.TitleItem?.alias).Distinct();
			this.SetData(item => this.ResultItemChanged?.Invoke(new ResultItemChangedEventArgs(Recipes.Where(o => item == o.TitleItem?.alias))), 
