...
internal partial class SqlParser
{
	...
	///     [ FOR UPDATE ]
	/// </summary>
	private IDataReader ParseSelect()
	{
		...
		if (from.Type == TokenType.EOF || from.Type == TokenType.SemiColon)
		{
			...
			return new DataReader(result.Select(x => x.IsDocument ? x.AsDocument : new AttributeDocument { [defaultName] = x }), null);
		}
		else
		{
			from.Expect("FROM");
	...
	/// Read collection name and parameter (in case of system collections)
	/// </summary>
	public static string ParseCollection(Tokenizer tokenizer, out string name, out AttributeValue options)
	{
		name = tokenizer.ReadToken().Expect(TokenType.Word, TokenType.String).Value;
		...
		return name + (options == null ? "" : "(" + JsonConvert.SerializeObject(options) + ")");
	}
}