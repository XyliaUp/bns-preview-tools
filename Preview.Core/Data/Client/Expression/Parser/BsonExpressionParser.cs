...
/// <summary>
/// Compile and execute simple expressions using AttributeDocuments. Used in indexes and updates operations. See https://github.com/mbdavid/LiteDB/wiki/Expressions
/// </summary>
internal class BsonExpressionParser
{
	...
	/// </summary>
	public static BsonExpression ParseFullExpression(Tokenizer tokenizer, ExpressionContext context, AttributeDocument parameters, DocumentScope scope)
	{
	...
	/// Start parse string into linq expression. Read path, function or base type bson values (int, double, bool, string)
	/// </summary>
	public static BsonExpression ParseSingleExpression(Tokenizer tokenizer, ExpressionContext context, AttributeDocument parameters, DocumentScope scope)
	{
	...
	/// </summary>
	public static BsonExpression ParseSelectDocumentBuilder(Tokenizer tokenizer, ExpressionContext context, AttributeDocument parameters)
	{
		...
		var arrValues = Expression.NewArrayInit(typeof(AttributeValue), fields.Select(x => x.Value.Expression).ToArray());
	...
	/// </summary>
	public static BsonExpression ParseUpdateDocumentBuilder(Tokenizer tokenizer, ExpressionContext context, AttributeDocument parameters)
	{
		var next = tokenizer.LookAhead();
	...
	/// </summary>
	private static BsonExpression TryParseDouble(Tokenizer tokenizer, AttributeDocument parameters)
	{
		string value = null;
		...
		if (value != null)
		{
			...
			var constant = Expression.Constant(AttributeValue.Create(number));
	...
	/// </summary>
	private static BsonExpression TryParseInt(Tokenizer tokenizer, AttributeDocument parameters)
	{
		string value = null;
		...
		if (value != null)
		{
			...
			if (isInt32)
			{
				var constant32 = Expression.Constant(AttributeValue.Create(i32));
			...
			var constant64 = Expression.Constant(AttributeValue.Create(i64));
	...
	/// </summary>
	private static BsonExpression TryParseBool(Tokenizer tokenizer, AttributeDocument parameters)
	{
		if (tokenizer.Current.Type == TokenType.Word && (tokenizer.Current.Is("true") || tokenizer.Current.Is("false")))
		{
			...
			var constant = Expression.Constant(AttributeValue.Create(boolean));
	...
	/// </summary>
	private static BsonExpression TryParseNull(Tokenizer tokenizer, AttributeDocument parameters)
	{
		if (tokenizer.Current.Type == TokenType.Word && tokenizer.Current.Is("null"))
		{
			var constant = Expression.Constant(AttributeValue.Null);
	...
	/// </summary>
	private static BsonExpression TryParseString(Tokenizer tokenizer, AttributeDocument parameters)
	{
		if (tokenizer.Current.Type == TokenType.String)
		{
			var bstr = AttributeValue.Create(tokenizer.Current.Value);
	...
	/// </summary>
	private static BsonExpression TryParseDocument(Tokenizer tokenizer, ExpressionContext context, AttributeDocument parameters, DocumentScope scope)
	{
	...
	/// </summary>
	private static BsonExpression TryParseArray(Tokenizer tokenizer, ExpressionContext context, AttributeDocument parameters, DocumentScope scope)
	{
	...
	/// </summary>
	private static BsonExpression TryParseParameter(Tokenizer tokenizer, ExpressionContext context, AttributeDocument parameters, DocumentScope scope)
	{
		if (tokenizer.Current.Type != TokenType.At) return null;

		var ahead = tokenizer.LookAhead(false);

		if (ahead.Type == TokenType.Word || ahead.Type == TokenType.String || ahead.Type == TokenType.Int)
		{
	...
	/// </summary>
	private static BsonExpression TryParseInnerExpression(Tokenizer tokenizer, ExpressionContext context, AttributeDocument parameters, DocumentScope scope)
	{
	...
	/// </summary>
	private static BsonExpression TryParseMethodCall(Tokenizer tokenizer, ExpressionContext context, AttributeDocument parameters, DocumentScope scope)
	{
	...
	/// <summary>
	/// Implement a JSON-Path like navigation on AttributeDocument. Support a simple range of paths
	/// </summary>
	private static Expression ParsePath(Tokenizer tokenizer, Expression expr, ExpressionContext context, AttributeDocument parameters, HashSet<string> fields, ref bool isImmutable, ref bool useSource, ref bool isScalar, StringBuilder src)
	{
		var ahead = tokenizer.LookAhead(false);
	...
	/// </summary>
	private static BsonExpression TryParseFunction(Tokenizer tokenizer, ExpressionContext context, AttributeDocument parameters, DocumentScope scope)
	{
		if (tokenizer.Current.Type != TokenType.Word) return null;
		if (tokenizer.LookAhead().Type != TokenType.OpenParenthesis) return null;
	...
	/// MAP(items[*] => @.Name)
	/// </summary>
	private static BsonExpression ParseFunction(string functionName, BsonExpressionType type, Tokenizer tokenizer, ExpressionContext context, AttributeDocument parameters, DocumentScope scope)
	{
	...
	private static string ReadField(Tokenizer tokenizer, StringBuilder source)
	{
		...
		else if (tokenizer.Current.Type == TokenType.Word || tokenizer.Current.Type == TokenType.String)
		{
	...
	public static string ReadKey(Tokenizer tokenizer, StringBuilder source)
	{
		...
		{
			key = token.Expect(TokenType.Word, TokenType.String, TokenType.Int).Value;
		}
	...
	{
		// convert AttributeValue into Boolean
		var boolLeft = Expression.Property(left.Expression, typeof(AttributeValue), "AsBoolean");
		var boolRight = Expression.Property(right.Expression, typeof(AttributeValue), "AsBoolean");

		var expr = type == BsonExpressionType.And ?
			Expression.AndAlso(boolLeft, boolRight) :
			Expression.OrElse(boolLeft, boolRight);

		// and convert back Boolean to AttributeValue
		var ctor = typeof(AttributeValue)
			.GetConstructors()
	...
	{
		// convert AttributeValue into Boolean
		var boolTest = Expression.Property(test.Expression, typeof(AttributeValue), "AsBoolean");
