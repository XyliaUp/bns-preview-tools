...
public class AttributeConverter
{
    ...
    public static object Convert(string value, Type type, IDataProvider provider)
    {
        ...
        else if (type.IsGenericType && type.GetGenericTypeDefinition() == typeof(Ref<>)) return Activator.CreateInstance(type, value, null  /*provider*/);


        if (TypeCode.TryGetValue(type, out var code)) return ConvertBack(value, new AttributeDefinition() { Type = code }, provider);
