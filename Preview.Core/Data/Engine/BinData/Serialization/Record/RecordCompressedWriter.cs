...
internal unsafe class RecordCompressedWriter
{
	...
	public void BeginWrite(DataArchiveWriter writer)
	{
	...
	public void EndWrite(DataArchiveWriter writer)
	{
#if ENABLE_ISA
        if (_dataFilled >= 0)
#else
	...
	public void WriteRecord(DataArchiveWriter writer, ReadOnlySpan<byte> data, ReadOnlySpan<byte> lookupBuffer)
	{
	...
	private void BeginCompressedBlock(DataArchiveWriter writer)
	{
		_lowKey = 0;
		_highKey = 0;
		_blockSize = 0;
		...
		_blockBegin = (int)writer.Position;
		writer.Write(EmptyBlockHeader);

#if ENABLE_ISA
        _dataFilled = 0;
#else
		_deflateStream = new DeflaterOutputStream(writer, new Deflater(6)) { IsStreamOwner = false };
#endif
	}

	private void EndCompressedBlock(DataArchiveWriter writer)
	{
		_blockCount++;
        ...
        if (size == 0)
             throw new Exception("Failed to compress buffer");
		...
		var afterBlockPosition = (int)writer.Position;
