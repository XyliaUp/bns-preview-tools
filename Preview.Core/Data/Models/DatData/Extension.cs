...
namespace Xylia.Preview.Data.Models.DatData
{
	public static class Extension
    {
        ...
        {
            Parallel.ForEach(bNSDat.FileTableList, df =>
            {
                string path = OutPath + "\\" + df.FilePath;
        ...
        public static byte[] ExtractBin(this BNSDat bNSDat)
        {
            ...
            Trace.WriteLine("开始读入文件");

            var file = bNSDat.FileTableList.Find(f => f.FilePath.RegexMatch(".*?.bin"));
            ArgumentNullException.ThrowIfNull(file);
            file.Decrypt();

            Trace.WriteLine($"读入文件（{file.FilePath}）完成 , {DateTime.Now.Subtract(dt).TotalSeconds:0.0s}");
            return file.Data;
        }
		#endregion
