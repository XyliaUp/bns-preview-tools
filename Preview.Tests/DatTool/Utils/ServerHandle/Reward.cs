...
public class Reward : TableHandle
{
    protected override void Fix(XmlElement record)
    {
        if (record.Attributes["rare-item-1"] != null && record.Attributes["rare-item-1-max"] is null)
        {
            ...
            for (int i = 1; i <= 40; i++)
            {
                var item = record.Attributes["rare-item-" + i];
                if (item != null)
                {
                    if (item.Value.Contains("Grocery_Festival_", StringComparison.OrdinalIgnoreCase)) record.SetAttribute("rare-item-prob-weight-" + i, "1");
