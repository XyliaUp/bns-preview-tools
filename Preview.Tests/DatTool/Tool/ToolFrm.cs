...
public partial class ToolFrm : Form
{
	...
	private void Btn_Compare_Click(object sender, EventArgs e)
	{
		...
		try
		{
			...
			string StrA = textBox3.Text.UnCompress();
			string StrB = textBox4.Text.UnCompress();
				...
				for (int f = 0; f < StrA.Length; f++)
				{
					if (StrB.Length > f)
					{
						if (StrA[f] != StrB[f])
						{
							if (Same.Length != 0)
							{
								richOut.AppendText(Same.ToString() + "\n");
								Same.Clear();
							}
	...
	public void Output(StringBuilder diff1, StringBuilder diff2, int Location)
	{
		if (diff1.Length != 0)
		{
			richOut.AppendText($"第{Location + 1}位\t\t{diff1}\t\t\t{diff2}\n\n");
			diff1.Clear();
			diff2.Clear();
		}
	}
	...
	private static void HexTextChanged(Control HexText, Control LengthShow)
	{
		try
		{
			if (!string.IsNullOrWhiteSpace(HexText.Text))
				LengthShow.Text = $"长度：{HexText.Text.UnCompress().Length / 2}";
	...
	private void button6_Click(object sender, EventArgs e)
	{
		...
		{
			if (line.StartsWith("ItemSkill_", StringComparison.OrdinalIgnoreCase))
			{
				if (data.Count != 0)
				{
		...
		else if (radioButton4.Checked)
		{
			...
			//{
			//	MessageBox.Show($"数量不一致 (info:{ info.Count },records:{ records.Count })");
			//	return;
			//}
		}
		else
		{
			MessageBox.Show($"模式无效");
			return;
		}
	...
	private void button4_Click(object sender, EventArgs e) => richTextBox1.Text = CreateEnum.Instance(richTextBox1.Text?.Trim());
	#endregion
}